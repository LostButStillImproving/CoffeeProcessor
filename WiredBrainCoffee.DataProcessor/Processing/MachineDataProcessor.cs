...
namespace WiredBrainCoffee.DataProcessor.Processing
{
    ...
    {
        if (!ItemIsNewer(dataItem)) return;
        if (!ContainsKey(dataItem)) _countPerCoffeeType.Add(dataItem.CoffeeType, 1);
        else
        {
    ...
    private bool ContainsKey(MachineDataItem dataItem)
    {
        return _countPerCoffeeType.ContainsKey(dataItem.CoffeeType);
    }

    private bool ItemIsNewer(MachineDataItem dataItem)
    {
        return _previousItem is null || dataItem.CreatedAt > _previousItem.CreatedAt;
    }
